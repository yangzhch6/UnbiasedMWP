...
def logic_cross_entropy(logic, inter):
    ...
    logic_num = torch.where(inter_flat == -1, zeros, ones).sum()
    ...
    loss_weight = torch.where(inter_flat == 0, 0.06*mask, mask.float())
    ...
    correct = (predicts == inter) * mask.view(*inter.size())
    accurate_count = correct.sum()

    return loss, accurate_count, logic_num
